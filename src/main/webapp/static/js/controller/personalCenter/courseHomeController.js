myApp.controller('courseHomeController', function ($scope, $http) {
    $scope.baseurl = getApiUrl();
    var storage;
    $scope.FilterPanel = [];

    if (!window.localStorage) {
        alert("浏览器支持localstorage");
    } else {
        storage = window.localStorage;
    }
    //将tab加入缓存
    storage.setItem("Selected", 'courseHome');
    $scope.SkipIndex = function () {
        window.location.href =  $scope.baseurl;
    }
    //校区选择
    $scope.schoolBox = function () {
        $http({
            method: "post",
            url:  $scope.baseurl + "/campus/show/list",
            dataType: "json",
            data: JSON.stringify($scope.FilterPanel),
            headers: {"Content-Type": "application/json;charset=utf-8"}
        }).then(function (data) {
            var resdata = angular.fromJson(data);
            if (resdata.data.code == 0) {//成功
                $scope.schoolBoxList = new Array();
                $scope.schoolBoxList = resdata.data.data;
            } else {
                alert(resdata.data.info);
            }
        });
    }
    $scope.schoolBox();
    $scope.SubSchool = function (schoolId) {
        //加入缓存
        storage.setItem("schoolId", schoolId);
        //跳转
        window.location.href = $scope.baseurl + "/user/home#/chooseCourses";
    }
});

//边框变色
$(".mrcb-campusBox").mouseover(
    function () {
        $(".mrcb-inside").css("border", "2px solid #00b7ee")
    }
);
$(".mrcb-campusBox").mouseout(
    function () {
        $(".mrcb-inside").css("border", "2px solid #e5e5e5")
    }
)